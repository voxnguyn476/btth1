...
struct pso{
    long long t,m; // t: tu so ; m: mau so
};
...
// lần lượt tương ứng "Đã hợp lệ" hoặc "Chưa hợp lệ".
...
void Nhap(pso &a){
    ...
    while(!c){
        cin >>a.t>>a.m;
        c=Hople(a);
        if(!c) cout <<"Phan so nhap vao khong hop le. Vui long nhap lai.\n";
    }
}
//Hàm Nhap là hàm void dùng để lưu phân số được nhập từ bàn phím vào biến có Cấu trúc pso
// được khởi tạo ban đầu trong hàm main (gọi là biến a). Hàm sẽ nhập tử và mẫu của a và
// kiểm tra xem "phân số" a có hợp lệ không, nếu không sẽ cho nhập lại đến hợp lệ thì kết thúc hàm.
